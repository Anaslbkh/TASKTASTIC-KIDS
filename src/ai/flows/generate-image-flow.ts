...
export async function generateImageFromPrompt(
  ...
  try {
    console.log(
      `generateImageFromPrompt: Generating image for prompt: "${input.prompt}"`
    );
    ...
    if (
      ...
      for (const part of response.candidates[0].content.parts) {
        if (part.text) {
          ...
          console.log(
            "generateImageFromPrompt: Received text part:",
          ...
          console.log(
            "generateImageFromPrompt: Successfully generated image data URI."
          );
        }
      }
    }
    ...
    if (imageUrl) {
      ...
      const warningMessage = textResponse
        ? `Image generation did not return a media URL. Text response received: ${textResponse.substring(
            0,
            200
          )}...`
    ...
    console.error(
      "Error in generateImageFromPrompt. Prompt was:",
      input.prompt,
      "Error:",
      error
    );
